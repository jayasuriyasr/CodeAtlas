...
## Architecture Specification v10.1
...
**Status:** Executed. Every Cypher statement in §11 has been run against Neo4j Kernel 5.26.0 (community), and §0.2's four unknowns are closed — see §0.0. Constants marked † remain unvalidated starting points: this revision fixes defects, and moves no threshold.

---

## 0. Status

### 0.0 Changes from v10.0

Three defects, found by **running** v10.0 rather than by reading it. Each is
listed with the run that produced it; all three were silent — none raised, and
two produced a graph or a metric that looked healthy while being wrong.

| # | Defect | Fix |
|---|---|---|
| **R1** | **§4.4's `resolve_moves` computes a UID that was never written.** It derives `old_uid` from the *new* file's `qualified_name` against the *old* path. §3.2 derives qualified names from the module path, so a move changes them — the two sentences cannot both hold. §11.2 2a is a `MATCH`, so the remap updates nothing and raises nothing; the upsert then creates the symbols fresh and step 5 detach-deletes the old ones, taking every inbound `CALLS` edge with them. `move.unresolvable` stays zero throughout, because a `ParsedFile` *was* found. This is the outcome §4.4 calls "worse than an error because it looks like an answer", reached by the mechanism built to prevent it. | Re-parse the same bytes at the old path (§4.4). `ParsedFile` retains `source` (§3.4). The same defect at its second site — §4.5's undetected-move criterion, which made `move.undetected` unable to leave zero — is corrected to match on the unqualified `name`. |
| **R2** | **§3.5's framework edge types cannot be written or traversed.** §11.2 3b's allowlist held four types; §3.5 names eight. `INVOKES_HOOK`, `RENDERS`, `HAS_FIELD` and `USES_SERIALIZER` could not be written, and §11.1's `MATCH` would not have traversed them if they had been. An edge written and never read is worse than one not written: the rows exist, and the question they answer returns nothing. | The allowlist, §11.1's `MATCH` and §5.2's weight table all cover the eight types §3.5 names. |
| **R3** | **§11.3's `symbol_origin` index cannot serve §11.2 steps 4, 6a or 6b.** The index is `(repo_id, origin_path, epoch)`; those three statements constrain only the first two. **Neo4j will not seek a composite index on a prefix of its properties**, so all three fell back to full scans — the exact defect B8 names, produced by the specification's own statements. Only step 5, which also constrains `epoch`, used the index. | Add `s.epoch <= $epoch` to steps 4, 6a and 6b. It is a tautology at that point in the sequence, and it completes the index's property set. |

**Runs cited.**

R1, on a move of `pkg/callee.py` to `pkg/moved.py`:

```
stored at old path:   uid=44a268a7ab97dd61b9ba  qname='pkg.callee.helper'
parsed at new path:   uid=74c6fda492a325c56de3  qname='pkg.moved.helper'
...
R3, hinting the two-property prefix of `symbol_origin`:
...
and the plans for the statements as written, on a 2,000-node / 12,000-edge graph:

| Statement | v10.0 operator | v10.1 operator |
|---|---|---|
| step 4 | `NodeByLabelScan` | `NodeIndexScan` on `symbol_origin` |
| step 5 | `NodeIndexSeek` on `symbol_origin` | unchanged |
| step 6a | `NodeByLabelScan` | `NodeIndexSeek` on `symbol_origin` |
| step 6b | `UndirectedAllRelationshipsScan` | `NodeIndexSeek` on `symbol_origin` |

**What this revision does not do.** No threshold moves; every constant marked †
is still v10.0's guess (§12.10). Two findings are recorded and deliberately not
acted on: §11.1's `CALL { WITH … }` is deprecated on 5.26 in favour of
`CALL (seed) { … }` and will break on a future major, and §3.4.3's overflow
split still has nowhere to put a symbol's second chunk, since §3.3 gives a
symbol exactly one `code_vec`. Both are in `FINDINGS.md` with their runs.
...
Each answered by executing one statement against Neo4j Kernel 5.26.0 (community).
Full output: `tests/step_01_ground_truth/PROBE_RESULTS.md`.

| Question | Answer |
|---|---|
| `$edge_weights[rel]` — dynamic map key on a parameter | **Yes.** `rel='CALLS'` returns `1.0`; a missing key returns null and `coalesce` covers it |
| `COUNT { … }` on the deployed minor | **Yes** — legal in a projection *and* on the right of `SET` |
| `NOT EXISTS { MATCH … }` | **Yes** — admits the free case, rejects the collision |
| Vector-index DDL: parentheses? | **`ON (s.code_vec)`**, and a written vector round-trips through `db.index.vector.queryNodes` |
| tree-sitter TSX node types | **Yes** — 76 named types over a real route + component, zero parse errors, every assumed type present |

`MERGE (a)-[:A\|B]->(b)` is rejected, as §11.2 3b requires:
`A single relationship type must be specified for MERGE`. The per-type split is
required, not defensive.

**Still unverified:** every threshold in §8.2 and every constant marked †. Seven
of the thirteen §8.2 rows now have measured values; the six that do not need a
live LLM. None of the thresholds has been re-tuned.
...
1. **Unit = one symbol** (function or method). `ParsedFile.symbols` is the only collection; each symbol carries its own `chunk_text` (T6). `ParsedFile` also retains `source`, the raw bytes it was parsed from, because §4.4's remap must re-parse them at the old path.
...
def resolve_moves(repo: str, moves: list[Move],
                  ...
                  adapter_for: Callable[[str], LanguageAdapter | None]) -> list[Remap]:
    """The new UID comes from the new file's parsed symbols. The old one cannot.

    `arity` and `ordinal` survive a move. **`qualified_name` does not**: §3.2
    derives it from the module path, so `pkg/callee.py::helper` is
    `pkg.callee.helper` and becomes `pkg.moved.helper` the instant the file
    moves. Computing old_uid from the new qualified_name against the old path
    yields a UID that was never written; §11.2 2a then matches nothing, every
    remap is a silent no-op, and the move severs exactly the inbound edges this
    mechanism exists to preserve. Nothing reports it — `move.unresolvable` stays
    zero because a ParsedFile *was* found. (v10.0 defect, §0.0.)
    ...
    identical bytes through one adapter give identical symbol order. Cost is one
    extra local parse per moved file (~5ms, §4).
    """
    out = []
    for mv in moves:
        ...
        as_old = adapter.parse(repo, mv.old_path, parsed.source) if adapter else None
        if as_old is None or len(as_old.symbols) != len(parsed.symbols):
            metrics.incr("move.requalify_failed")   # refuse; never guess
            continue
        for old_sym, new_sym in zip(as_old.symbols, parsed.symbols):
            # `name` is exempt for :Module symbols: a module's name IS its
            # filename, so it is expected to change across a move.
            if (old_sym.kind, old_sym.arity, old_sym.ordinal) != (
                    new_sym.kind, new_sym.arity, new_sym.ordinal) or (
                    old_sym.kind != "module" and old_sym.name != new_sym.name):
                metrics.incr("move.requalify_failed")
                continue
            out.append(Remap(
                old_uid=old_sym.uid, new_uid=new_sym.uid,
...
async def full_reconcile(self, repo: str, epoch: int | None = None) -> None:
    ...
    # unqualified `name` was a move no live signal caught. No stored previous-
    # reconcile state is required.
    #
    # NOT qualified_name. It is path-derived (§3.2), so a pure move never
    # matches and this counter reads zero forever -- deferring §10.2's
    # import-path repair on evidence that was never collected. Same defect as
    # §4.4's, second site. (v10.0 defect, §0.0.)
...
Edge weights†: `CALLS 1.0`, `DEFINES 0.8`, `DISPATCHES_TO 0.9`, `INVOKES_HOOK 0.9`, `RENDERS 0.8`, `USES_SERIALIZER 0.7`, `HAS_FIELD 0.6`, `IMPORTS 0.4`. Every type §3.5 names has a weight, or expansion would score it at the coalesce default while §11.1 declined to traverse it at all.
...
-- allowlist {CALLS, IMPORTS, DEFINES, DISPATCHES_TO, INVOKES_HOOK, RENDERS,
-- HAS_FIELD, USES_SERIALIZER}, never from parsed input. The allowlist must
-- cover every type §3.5 names: v10.0 listed four, so the framework layer's
-- other four could not be written -- and §11.1 would not have traversed them
-- had they been. A fixed list of eight is exactly as safe as one of four;
-- what matters is that it is fixed, not that it is short.
...
--
-- `s.epoch <= $epoch` is not a filter. Neo4j will not seek a composite index
-- on a PREFIX of its properties, so a statement constraining only repo_id and
-- origin_path cannot use symbol_origin at all and falls back to a full scan.
-- The predicate is a tautology here: 3a has just stamped every symbol in the
-- batch with $epoch, and anything else at these paths is older. Steps 6a and
-- 6b carry it for the same reason. (v10.0 defect, §0.0.)
MATCH (s:Symbol)
...
It has now been run. The four Cypher constructs are verified (§0.2), and running
it found three defects that reading it had not — each silent, each in a
mechanism built to prevent the failure it caused (§0.0). Every threshold is
still a guess, and §12.0 still poses a market question this document cannot
answer.

The specification was internally consistent and honestly bounded, and was not
correct. That is the argument for the Week 1 scaffold, made by the scaffold
itself: three of these defects were unreachable by reading, and two of them
produced a graph or a metric that looked healthy while being wrong.
